...
def load_vae_encoder():
    ...
    model.load_state_dict(torch.load('/home/selab/darshan/git-repos/model/var_encoder_model.pth'))
    model.eval()
    return model
...
def batch_predict(images, model, transform):
    ...
    batch = torch.stack([transform(PilImage.fromarray(image)) for image in images]).to(device)
...
def main():
    image_path = "/home/selab/darshan/git-repos/dataset/town7_dataset/test/town7_000260.png"# Adjust to your image path
