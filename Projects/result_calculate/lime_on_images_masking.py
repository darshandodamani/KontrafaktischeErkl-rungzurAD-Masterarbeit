...
train_df = train_df[train_df["Prediction (After Masking)"].notna()]
test_df = test_df[test_df["Prediction (After Masking)"].notna()]
...
def summarize_results(df, total_go, total_stop):
    ...
    for _, row in df.iterrows():
        ...
        prediction_after = row["Prediction (After Masking)"]
...
summary_table.to_csv("Projects/result_calculate/lime_image_results_summary.csv", index=False)
