...
num_epochs = 20  # Logistic regression is not epoch-based, but for consistency, we'll use 1 epoch for evaluation
learning_rate = 0.001  # Not used in logistic regression, but present for consistency
...
logistic_model = linear_model.LogisticRegression(max_iter=500)  # Instantiate the Logistic Regression model with a max iteration limit
logistic_model.fit(X_train, y_train)  # Train the model on the training data
y_pred_logistic = logistic_model.predict(X_test)  # Predict labels for the test set

# Evaluation metrics
accuracy = accuracy_score(y_test, y_pred_logistic)  # Calculate accuracy of the model
precision = precision_score(y_test, y_pred_logistic, average='weighted')  # Calculate precision of the model
recall = recall_score(y_test, y_pred_logistic, average='weighted')  # Calculate recall of the model
f1 = f1_score(y_test, y_pred_logistic, average='weighted')  # Calculate F1 score of the model
...
# Confusion Matrix
conf_matrix = confusion_matrix(y_test, y_pred_logistic)  # Generate the confusion matrix
...
plt.savefig("plots/logistic_regression_confusion_matrix.png")  # Save the confusion matrix plot
plt.show()

# ROC Curve and AUC
fpr, tpr, _ = roc_curve(y_test, y_pred_logistic, pos_label=1)  # Calculate false positive rate and true positive rate for the ROC curve
...
plt.savefig("plots/logistic_regression_roc_curve.png")  # Save the ROC curve plot
...
torch.save(logistic_model, "model/epochs_500_latent_128/logistic_regression_model.pth")  # Save the trained logistic regression model
...
plt.savefig("plots/classifier_plots/logistic_regression_confusion_matrix.png")  # Save the confusion matrix plot
plt.savefig("plots/classifier_plots/logistic_regression_roc_curve.png")  # Save the ROC curve plot
