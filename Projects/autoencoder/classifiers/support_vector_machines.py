...
num_epochs = 20  # SVM is not epoch-based, but for consistency, we'll use 1 epoch for evaluation
learning_rate = 0.001  # Not used in SVM, but present for consistency
...
# Train SVM Model
svm_model = svm.SVC(probability=True)  # Instantiate the SVM model with probability enabled for ROC curve
svm_model.fit(X_train, y_train)  # Train the model on the training data
y_pred_svm = svm_model.predict(X_test)  # Predict labels for the test set

# Evaluation metrics
accuracy = accuracy_score(y_test, y_pred_svm)  # Calculate accuracy of the model
precision = precision_score(y_test, y_pred_svm, average='weighted')  # Calculate precision of the model
recall = recall_score(y_test, y_pred_svm, average='weighted')  # Calculate recall of the model
f1 = f1_score(y_test, y_pred_svm, average='weighted')  # Calculate F1 score of the model
...
# Confusion Matrix
conf_matrix = confusion_matrix(y_test, y_pred_svm)  # Generate the confusion matrix
...
plt.savefig("plots/svm_confusion_matrix.png")  # Save the confusion matrix plot
# ROC Curve and AUC
fpr, tpr, _ = roc_curve(y_test, svm_model.predict_proba(X_test)[:, 1], pos_label=1)  # Calculate false positive rate and true positive rate for the ROC curve
...
plt.savefig("plots/svm_roc_curve.png")  # Save the ROC curve plot
...
import joblib
joblib.dump(svm_model, "model/epochs_500_latent_128/svm_model.pkl")  # Save the trained SVM model
...
plt.savefig("plots/classifier_plots/svm_confusion_matrix.png")  # Save the confusion matrix plot
plt.savefig("plots/classifier_plots/svm_roc_curve.png")  # Save the ROC curve plot
print("SVM model saved successfully!")  # Print success message

# Note: SVM does not have epoch-based training, so train_losses and train_accuracies are not applicable.
