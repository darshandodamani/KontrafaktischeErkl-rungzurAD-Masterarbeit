...
num_epochs = 20  # KNN is not epoch-based, but for consistency, we'll use 1 epoch for evaluation
learning_rate = 0.001  # Not used in KNN, but present for consistency
...
knn_model = neighbors.KNeighborsClassifier(n_neighbors=5)  # Instantiate the KNN model with 5 neighbors
knn_model.fit(X_train, y_train)  # Train the model on the training data
y_pred_knn = knn_model.predict(X_test)  # Predict labels for the test set

# Evaluation metrics
accuracy = accuracy_score(y_test, y_pred_knn)  # Calculate accuracy of the model
precision = precision_score(y_test, y_pred_knn, average='weighted')  # Calculate precision of the model
recall = recall_score(y_test, y_pred_knn, average='weighted')  # Calculate recall of the model
f1 = f1_score(y_test, y_pred_knn, average='weighted')  # Calculate F1 score of the model
...
# Confusion Matrix
conf_matrix = confusion_matrix(y_test, y_pred_knn)  # Generate the confusion matrix
...
plt.savefig("plots/knn_confusion_matrix.png")  # Save the confusion matrix plot
plt.show()  # Display the plot

# ROC Curve and AUC
fpr, tpr, _ = roc_curve(y_test, knn_model.predict_proba(X_test)[:, 1], pos_label=1)  # Calculate false positive rate and true positive rate for the ROC curve
...
plt.savefig("plots/knn_roc_curve.png")  # Save the ROC curve plot
...
joblib.dump(knn_model, "model/epochs_500_latent_128/knn_model.pkl")  # Save the trained KNN model
...
plt.savefig("plots/classifier_plots/knn_confusion_matrix.png")  # Save the confusion matrix plot
plt.savefig("plots/classifier_plots/knn_roc_curve.png")  # Save the ROC curve plot
print("KNN model saved successfully!")  # Print success message

# Note: KNN does not have epoch-based training, so train_losses and train_accuracies are not applicable.
