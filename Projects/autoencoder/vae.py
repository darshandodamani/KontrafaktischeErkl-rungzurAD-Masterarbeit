    ...
    def __init__(self, img_dir, csv_file, transform=None):
        ...
        with open(csv_file, 'r') as file:
            ...
            for row in reader:
                ...
                label = 0 if row['label'] == 'STOP' else 1  # Convert labels to binary (0 for STOP, 1 for GO)
...
def train(model, trainloader, optimizer, epoch):
    ...
    for batch_idx, (x, _) in enumerate(trainloader):
        ...
        if batch_idx % 10 == 0 or batch_idx == 0:
            ...
            print(f'MSE Loss: {mse_loss.item()}, KL Loss: {kl_loss.item()}, Total Loss: {loss.item()}')
            
        # Save and visualize a few reconstructed images
        if batch_idx == 0:  # Just do this for the first batch of each epoch
            plt.figure(figsize=(8, 4))
            for i in range(min(4, x.size(0))):  # Visualize the first 4 images in the batch
...
def main():
    ...
    train_data = CustomImageDatasetWithLabels(data_dir+'train', data_dir+'train/labeled_train_data_log.csv', transform=train_transforms)
    test_data = CustomImageDatasetWithLabels(data_dir+'test', data_dir+'test/labeled_test_data_log.csv', transform=test_transforms)
