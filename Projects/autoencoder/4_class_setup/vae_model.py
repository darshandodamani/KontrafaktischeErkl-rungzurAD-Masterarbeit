#location: Projects/autoencoder/4_class_setup/vae_model.py
import os
import sched
import sys
import torch
...
from vae_encoder import VariationalEncoder
from vae_decoder import VariationalDecoder
...
class VariationalAutoencoder(nn.Module):
    def __init__(self, latent_dims, num_epochs):  # Add num_epochs as a parameter
        ...
        self.decoder = VariationalDecoder(latent_dims, num_epochs)
...
def save_reconstructions(model, trainloader, epoch):
    ...
    with torch.no_grad():
        for batch_idx, data in enumerate(trainloader):
            ...
            save_dir = f"results/reconstructed_images_{NUM_EPOCHS}_epochs_{LATENT_SPACE}_LF/"
...
def visualize_latent_space(z_values, labels, save_path="results/latent_space_4_classes.png"):
    """
    Visualize the latent space using t-SNE for dimensionality reduction.

    Args:
        z_values (torch.Tensor): The latent space vectors (batch_size, latent_dims).
        labels (list or np.ndarray): The class labels corresponding to each vector.
        save_path (str): Path to save the visualization plot.
    """
    # Convert z_values to numpy if it's a PyTorch tensor
    if isinstance(z_values, torch.Tensor):
        z_values = z_values.cpu().detach().numpy()

    # Perform t-SNE for dimensionality reduction to 2D
    tsne = TSNE(n_components=2, random_state=42)
    z_embedded = tsne.fit_transform(z_values)
    ...
    plt.colorbar(scatter, ticks=[0, 1, 2, 3], label="Class Labels")
    plt.title("t-SNE Visualization of Latent Space for 4 Classes")
...
def main():
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        # Training phase
        train_loss, train_mse_loss, train_kl_loss, z_values_tensor, all_image_paths = train(
        ...
        # Visualize latent space for the current epoch
        latent_vectors = z_values_tensor  # Use latent vectors from training
        class_labels = [
            trainloader.dataset.dataset.data[idx][1] for idx in trainloader.dataset.indices
        ]  # Access labels from the original dataset
        visualize_latent_space(
            latent_vectors,
            class_labels,
            save_path=f"results/latent_space_epoch_{epoch + 1}.png",
        )
        ...
        if epoch % 50 == 0:
            model_checkpoint_path = f"model/checkpoints/{NUM_EPOCHS}_epochs_LATENT_SPACE_LF/model_checkpoint_epoch_{epoch}.pth"
            os.makedirs(os.path.dirname(model_checkpoint_path), exist_ok=True)
            torch.save(model.state_dict(), model_checkpoint_path)
            print(f"Model checkpoint saved at epoch {epoch}")

    # Final latent space visualization
    print("Visualizing latent space for the entire training set...")
    all_latent_vectors = []
    all_class_labels = []

    # Collect latent vectors and labels from the training set
    with torch.no_grad():
        for x, labels, image_paths in trainloader:
            x = x.to(device)
            _, _, _, z = model(x)  # Encode input images to latent space
            all_latent_vectors.append(z.cpu())
            all_class_labels.extend(labels.numpy())

    latent_vectors = torch.cat(all_latent_vectors, dim=0)
    class_labels = np.array(all_class_labels)

    # Visualize the latent space
    visualize_latent_space(latent_vectors, class_labels, save_path="results/latent_space_final.png")
