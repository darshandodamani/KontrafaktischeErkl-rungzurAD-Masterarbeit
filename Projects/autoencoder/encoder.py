...
class VariationalEncoder(nn.Module):
    def __init__(self, latent_dims):
        ...
        self.encoder_layer2 = nn.Sequential(
            nn.Conv2d(32, 64, 3, stride=2),  # Input: 32 channels, Output: 64 channels
            nn.BatchNorm2d(64),
            nn.LeakyReLU())
        ...
        self.linear = nn.Sequential(
            nn.Linear(9 * 4 * 256, 1024),  # Adjust the input size according to the flattened dimension
            nn.LeakyReLU())
    ...
    def load(self):
        try:
            self.load_state_dict(torch.load(self.model_file, map_location=torch.device(device)))
